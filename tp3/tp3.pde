...
//link del video - https://youtu.be/cLRnkD1jGk0
...
for (int i = 0; i < cantcir; i++) {
 if (i % 2 == 0) {
  ...
  float diam = calculodiam(i); //funcion que retorna valor, los circulos se van achicando
...
void keyPressed() {        //reinicia cuando se presiona R/r
  if (key == 'r' || key == 'R') {
...
void reiniciar(boolean todo) {
  ...
  for (int i = 0; i < lineas; i++) {           //reestablece colores
    colores[i] = (i % 2 == 0) ? color(0) : color(255);
  }
}


float calculodiam(int i) {            //retorna el valor
